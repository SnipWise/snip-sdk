...
func main() {
	...
	// Ollama
	engineURL := env.GetEnvOrDefault("MODEL_RUNNER_BASE_URL", "http://localhost:11434/v1")
	chatModelId := env.GetEnvOrDefault("CHAT_MODEL", "qwen2.5:latest")

	compressorModelId := env.GetEnvOrDefault("COMPRESSOR_MODEL", "qwen2.5:1.5b")
