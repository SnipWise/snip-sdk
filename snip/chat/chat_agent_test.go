...
func TestAgentKind(t *testing.T) {
	...
	kind := agent.Kind()
	if kind != agents.Basic {
		t.Errorf("Kind() = %v, want %v", kind, agents.Basic)
	}
}
...
func TestAgentAskStream(t *testing.T) {
	t.Run("chatStreamFlow not initialized", func(t *testing.T) {
		...
		callback := func(chunk agents.ChatResponse) error {
			return nil
		}
...
func TestAgentKindValues(t *testing.T) {
	tests := []struct {
		...
		{"Basic kind", agents.Basic, "Basic"},
		{"Remote kind", agents.Remote, "Remote"},
		{"Tool kind", agents.Tool, "Tool"},
		{"Intent kind", agents.Intent, "Intent"},
		{"Rag kind", agents.Rag, "Rag"},
	}
