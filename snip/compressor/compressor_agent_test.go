...
func TestCompressorAgentGetKind(t *testing.T) {
	...
	if got := compressor.GetKind(); got != agents.Compressor {
		t.Errorf("GetKind() = %v, want %v", got, agents.Compressor)
	}
}
...
func TestCompressorAgentCompressText(t *testing.T) {
	t.Run("chatFlow not initialized", func(t *testing.T) {
		agent := &chat.ChatAgent{
			// ctx and chatFlow are not exported, test will need adjustment
		}
		compressor := &CompressorAgent{
			agent:             agent,
